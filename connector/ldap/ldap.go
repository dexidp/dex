...
func (c *ldapConnector) groups(ctx context.Context, user ldap.Entry) ([]string, error) {
	...
	for _, matcher := range c.GroupSearch.UserMatchers {
		...
		// If recursion is off, convert direct groups into names and return
		...
		// Recursive Search
		c.logger.Info("Recursive enabled for matcher", "groupAttr", matcher.GroupAttr)
		if matcher.RecursionGroupAttr == "" {
			return nil, fmt.Errorf(
				"ldap: recursionGroupAttr must be set when recursive=true on matcher %q", matcher.GroupAttr)
		}
		...
		for {
			...
			for _, group := range groups {
				...
				// If recursive is enabled, search for parent groups using the group's DN.
				parents, filter, err := c.queryGroups(ctx, matcher.RecursionGroupAttr, group.DN)
				if err != nil {
					return nil, err
				}
				if len(parents) == 0 {
					c.logger.Debug("no parents found for filter", "filter", filter)
				} else {
	...
	if err := c.do(ctx, func(conn *ldap.Conn) error {
		...
		if err != nil {
			if ldapErr, ok := err.(*ldap.Error); ok && ldapErr.ResultCode == ldap.LDAPResultNoSuchObject {
				c.logger.Info("ldap: search with filter %q returned no groups", filter)
				return nil
			}
