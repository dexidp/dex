...
func (h *Handler) introspectRefreshToken(ctx context.Context, token string) (*Introspection, error) {
	...
	refresh, err := tokens.LookupRefreshToken(ctx, h.Storage, h.ExpiryPolicy, h.Logger, nil, rToken)
	if err != nil {
	...
	return &Introspection{
		...
		Expiry:    refresh.CreatedAt.Add(h.ExpiryPolicy.RefreshStrategy(refresh.ConnectorID).AbsoluteLifetime()).Unix(),
