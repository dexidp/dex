...
func testAccessToken(t *testing.T) string {
	...
	issuer := tokens.NewIssuer(memory.New(logger), sig, *issURL, tokens.NewExpiryPolicy(time.Hour, nil, tokens.ExpiryCeilings{}, tokens.RefreshTokenDefaults{}, nil), time.Now, logger)
