...
func NewIssuer(storage storage.Storage, sig signer.Signer, issuerURL url.URL, expiry *ExpiryPolicy, now func() time.Time, logger *slog.Logger) *Issuer {
	return &Issuer{
		storage:      storage,
		signer:       sig,
		issuerURL:    issuerURL,
...
func (i *Issuer) SignIDToken(ctx context.Context, auth Authorization, accessToken, code string) (string, time.Time, error) {
	issuedAt := i.now()
	expiry := issuedAt.Add(i.expiryPolicy.IDTokensValidFor(auth.ConnectorID))
