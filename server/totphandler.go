...
func (s *Server) handleTOTPVerify(w http.ResponseWriter, r *http.Request) {
	...
	offlineSession, err := s.storage.GetOfflineSessions(authReq.Claims.UserID, authReq.ConnectorID)
	if err != nil {
	...
	case http.MethodPost:
		...
		if err := s.storage.UpdateOfflineSessions(offlineSession.UserID, offlineSession.ConnID, func(old storage.OfflineSessions) (storage.OfflineSessions, error) {
		...
		if err := s.storage.UpdateAuthRequest(authReq.ID, func(old storage.AuthRequest) (storage.AuthRequest, error) {
