...
func (c *conn) CreatePassword(p storage.Password) error {
	...
	_, err := c.Exec(`
		insert into password (
			email, hash, username, user_id, email_verified
		)
		values (
			$1, $2, $3, $4, $5
		);
	`,
		p.Email, p.Hash, p.Username, p.UserID, p.EmailVerified,
	)
	if err != nil {
		if c.alreadyExistsCheck(err) {
			return storage.ErrAlreadyExists
...
func (c *conn) UpdatePassword(email string, updater func(p storage.Password) (storage.Password, error)) error {
	return c.ExecTx(func(tx *trans) error {
		...
		_, err = tx.Exec(`
			update password
			set
				hash = $1, username = $2, user_id = $3, email_verified = $4
			where email = $5;
		`,
			np.Hash, np.Username, np.UserID, np.EmailVerified, p.Email,
		)
		if err != nil {
