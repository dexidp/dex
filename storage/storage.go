...
type AuthRequest struct {
	...
	// Resource indicators (RFC 8707) requested by the client. When non-empty,
	// the issued access token's audience is bound to these values rather than
	// to the client ID. Each value is an absolute URI.
	Resource []string
...
type AuthCode struct {
	...
	// Resource indicators (RFC 8707) carried over from the AuthRequest. Used to
	// bind the issued access token's audience when the code is exchanged.
	Resource []string
...
type RefreshToken struct {
	...
	// Resource indicators (RFC 8707) carried over from the initial request.
	// Refreshed access tokens keep this audience binding.
	Resource []string
...
type DeviceRequest struct {
	...
	Scopes []string
	// Resource indicators (RFC 8707) the device requests. Carried through the
	// authorization redirect to bind the issued access token's audience.
	Resource []string
	// The expire time
	Expiry time.Time
}
